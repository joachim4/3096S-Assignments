...
static uint16_t positionAddress = 0;
static uint8_t patterns[] = { 0b10101010, // 10101010 in binary
...
void TIM16_IRQHandler(void) {
	...
	// Assigning LED pin numbers to structure - easier to use
	struct LED ledAssign[] = { { GPIOB, GPIO_PIN_0 }, { GPIOB, GPIO_PIN_1 }, {
			GPIOB, GPIO_PIN_2 }, { GPIOB, GPIO_PIN_3 }, { GPIOB, GPIO_PIN_4 }, {
	...
	// Assigning address
	uint8_t bValue = read_from_address(positionAddress);

	for (int i = 0; i < 8; i++) {
		if (bValue & (1 << i)) {
			// Turning on the selected LED
			LL_GPIO_SetOutputPin(ledAssign[i].GPIO_Port, ledAssign[i].GPIO_Pin);
		} else {
			// Turning off the selected LED
			LL_GPIO_ResetOutputPin(ledAssign[i].GPIO_Port,
					ledAssign[i].GPIO_Pin);
		}
	}
	// Increment position
	positionAddress++;

	if (positionAddress >= sizeof(patterns)) {
		positionAddress = 0;
	}
