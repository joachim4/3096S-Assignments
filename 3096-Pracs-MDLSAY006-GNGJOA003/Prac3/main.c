...
int main(void)
{
  ...
  while (1)
  {
	...
	// ADC to LCD; TODO: Read POT1 value and write to LCD
	 ...
	 // Read ADC value using the given function
	    adc_val = pollADC();

	    // Get string of ADC value to print to LCD
	    char adc_line[16];
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	    ...
	    // We used the HAL Adc functions to start, convert and stop the adc
...
// Calculate PWM CCR value - Capture/Compare Register
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	...
	// Equation to calculate appropriate CCR value
	uint32_t val = (adc_val * ARRvalueRange) / ADCvalueRange;
