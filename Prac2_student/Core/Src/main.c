...
static uint8_t patterns[] = {
		0b10101010, // 10101010 in binary
		0b01010101, // 01010101 in binary
		0b11001100, // 11001100 in binary
		0b00110011, // 00110011 in binary
...
int main(void)
{
  ...
  uint16_t eepromAddress = 0;
      ...
      for (uint16_t i = 0; i < sizeof(patterns); i++) {
          write_to_address(eepromAddress++, patterns[i]);
      }


  uint32_t timerPeriod1s = 1000;
  uint32_t timerHalfPeriod = 500;
  uint32_t currentTimerPeriod = timerPeriod1s;
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  if (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0))
	          {
	              // Button is pressed, switch to a 0.5-second delay
	              currentTimerPeriod = timerHalfPeriod;
	              __HAL_TIM_SET_AUTORELOAD(&htim16, currentTimerPeriod);
	          }
	          else
	          {
	              // Button is not pressed, use the default 1-second delay
	              currentTimerPeriod = timerPeriod1s;
	              __HAL_TIM_SET_AUTORELOAD(&htim16, currentTimerPeriod);
	          }
...
void TIM16_IRQHandler(void)
{
	...
	// __HAL_TIM_CLEAR_FLAG(&htim16, TIM_FLAG_UPDATE);
	      uint8_t binaryValue = read_from_address(currentAddress);


	      uint8_t expectedValue = patterns[currentAddress];
	          if (binaryValue != expectedValue) {
	      ...
	      for (int i = 0; i < 8; i++) {
	              if (binaryValue & (1 << i)) {
	                  ...
	                  // For example, using LL_GPIO:
	                  LL_GPIO_SetOutputPin(leds[i].GPIO_Port, leds[i].GPIO_Pin);
	              } else {
	                  // Turn off the corresponding LED
	                  // For example, using LL_GPIO:
	                  LL_GPIO_ResetOutputPin(leds[i].GPIO_Port, leds[i].GPIO_Pin);
	              }
	          }

	      currentAddress++;

	      if (currentAddress >= sizeof(patterns)) {
	              currentAddress = 0;
	          }
